...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler
{
			...
			HttpHeaders headers, HttpStatus status, WebRequest request)
	{
		...
		List<Problem.Field> fields = bindingResult.getFieldErrors().stream().map(fieldError -> Problem.Field.builder()
				.name(fieldError.getField()).userMessage(fieldError.getDefaultMessage()).build())
				.collect(Collectors.toList());

		Problem problem = createProblemBuilder(status, problemType, detail).fields(fields).userMessage(detail).build();
